...
{
    /// <summary>
    /// This class is modified from the original implementation in System.Web.Mvc.DefaultModelBinder
    /// by RYounker to use the values passed into the UpdateModel Method when updating
    /// a collection rather than throwing them away, and using only the info provided
    /// in the Form data.  Unfortunately, due to the scoping of some of the objects and methods
    /// involved.  I was forced to duplicate the source code rather than derive from the class.
    /// It depends on Prototype1.Foundation.Web.Mvc.TypeHelpers, as well, but it has not been modified.
    /// 
    /// It has additionally been modified (SetProperty) to set strings to null instead of an empty string when the
    /// corresponding field value from the form has no data.  This allows the use of not null validators.
    /// 
    /// Modified CreateModel to use IDataContractFactory instead of Activator.CreateInstance
    /// </summary>
    public class CollectionPreservingModelBinder : IModelBinder
    {
        ...
        protected static void AddValueRequiredMessageToModelState(ControllerContext controllerContext, ModelStateDictionary modelState, string modelStateKey, Type elementType, object value)
        {
        ...
        protected internal virtual void BindComplexElementalModel(ControllerContext controllerContext, ModelBindingContext bindingContext, object model)
        {
        ...
        protected internal virtual object BindComplexModel(ControllerContext controllerContext, ModelBindingContext bindingContext)
        {
            ...
            if (model == null)
            {
                ...
                //mo - Invalid value will cause model to be null. We do not want fully bind meta properies if this occurs. Just leave
                if (model == null)
                {
                    return null;
                }
        ...
        public virtual object BindModel(ControllerContext controllerContext, ModelBindingContext bindingContext)
        {
            ...
            if (!String.IsNullOrEmpty(bindingContext.ModelName) && !bindingContext.ValueProvider.ContainsPrefix(bindingContext.ModelName))
            {
                ...
                if (bindingContext.FallbackToEmptyPrefix)
                {
                    ...
                    {
                        ModelName = controllerContext.Controller.ViewData.TemplateInfo.HtmlFieldPrefix,
        ...
        {
            //HACK: For some reason, when the raw value is a string array, and the destination type is a flags enum, it can only 
            //get the first value, rather than set all the selected flags. If we use a comma delimited string instead of a string array
            //the correct result is provided by the ValueProviderResult.ConverTo method call
            //proudly Hacked by Ryounker
            Type typeToTest = destinationType;
            if (destinationType.Name == typeof(Nullable<>).Name)
            {
                typeToTest = destinationType.GetGenericArguments()[0];
            }
            if (typeToTest.IsEnum && typeToTest.GetCustomAttributes(typeof(FlagsAttribute), false).Any())
            {
                valueProviderResult = new ValueProviderResult(valueProviderResult.AttemptedValue, valueProviderResult.AttemptedValue, valueProviderResult.Culture);
            }
            try
            {
        ...
        protected static string CreateSubIndexNameForExtraDotBeforeIndexer(string prefix, string index)
        {
            return String.Format(CultureInfo.InvariantCulture, "{0}.[{1}]", prefix, index);
        }
        ...
        protected static void GetIndexes(ModelBindingContext bindingContext, out bool stopOnIndexNotFound, out IEnumerable<string> indexes)
        {
            ...
            if (vpResult != null)
            {
                string[] indexesArray = (vpResult.ConvertTo(typeof(string)) as string).Split(new char[] { ',' });
        ...
        {
            Type type = bindingContext.Model != null ? bindingContext.Model.GetType() : bindingContext.ModelType;
            return new AssociatedMetadataTypeTypeDescriptionProvider(type).GetTypeDescriptor(type); //(bindingContext.ModelType);
        }
        ...
        {
            return GetUserResourceString(controllerContext, "PropertyValueInvalid") ?? "Invalid Value for property";
        }
        ...
        /// <summary>
        /// This method was modified from the original implementation in System.Web.Mvc
        /// by RYounker to use the values passed into the UpdateModel Method when updating
        /// a collection rather than throwing them away, and using only the info provided
        /// in the Form data.
        /// </summary>
        ...
        /// <returns></returns>
        protected internal virtual object UpdateCollection(ControllerContext controllerContext, ModelBindingContext bindingContext, Type elementType)
        {
            ...
            {
                //Added - RYounker - We know it's an IList at this point
                //Get each item from the list if there is one at its index.
                IList list = (IList)bindingContext.Model;
                int index;
                bool parsed = Int32.TryParse(currentIndex, out index);
                object item = (parsed && list != null && list.Count > index) ? list[index] : null;
                //end added code

                string subIndexKey = CreateSubIndexName(bindingContext.ModelName, currentIndex);
                if (!bindingContext.ValueProvider.ContainsPrefix(subIndexKey))
                    subIndexKey = CreateSubIndexNameForExtraDotBeforeIndexer(bindingContext.ModelName, currentIndex);
                var removeItemProperty = bindingContext.ValueProvider.GetValue(subIndexKey + ".RemoveItem");
                bool removeItem = false;
                if (removeItemProperty != null)
                {
                    removeItem = (bool)removeItemProperty.ConvertTo(typeof(bool));
                }
                ...
                if (!removeItem)
                {
                    ...
                    //Added - RYounker 
                    //setting the Model allows us to bind the new values to the old model
                    //thus preserving any values passed to the UpdateModel method.
                    //TODO: This will break down if a polymorphic collection's sort order changes
                    Type thisElementType = item != null ? item.GetType() : elementType;
                    var modelMetaData = ModelMetadataProviders.Current.GetMetadataForType(null, thisElementType);
                    if (item != null)
                        modelMetaData.Model = item;
        ...
        protected internal virtual object UpdateDictionary(ControllerContext controllerContext, ModelBindingContext bindingContext, Type keyType, Type valueType)
        {
            bool stopOnIndexNotFound;
            IEnumerable<string> indexes;
            GetIndexes(bindingContext, out stopOnIndexNotFound, out indexes);
