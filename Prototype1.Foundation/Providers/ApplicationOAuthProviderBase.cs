...
{
    public abstract class ApplicationOAuthProviderBase<T> : OAuthAuthorizationServerProvider
        where T : class, IApplicationUser
    {
        private readonly string _publicClientId;
        private readonly Func<UserManager<T>> _userManagerFactory;

        public ApplicationOAuthProviderBase(string publicClientId, Func<UserManager<T>> userManagerFactory)
        {
        ...
        public override async Task GrantResourceOwnerCredentials(OAuthGrantResourceOwnerCredentialsContext context)
        {
            using (var userManager = _userManagerFactory())
            {
                ...
                Guid userID;
                if (Guid.TryParse(user.Id, out userID) && userID != Guid.Empty)
                {
                    properties.Dictionary.Add("account_id", userID.ToString());
                }
